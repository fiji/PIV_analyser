 ...
 * 
 * <h4>Filtering</h4> The plugin returns also a stack made of the peak height
 * and a tentative signal/noise ratio for the analysis.
 * <p>
...
public class PIV_analyser implements PlugInFilter
{
	...
	/**
	 * Enumeration constant that stores all possible window size for this
	 * plugin. Because we use the Fast Hartley Transform of ImageJ, only square
	 * windows of power of 2 are possible.
	 */
	...
	/**
	 * Utility class used to store parameters specifying how to pair images for
	 * PIV analysis.
	 */
	 ...
	 * @return an array of ImagePlus containing: the image for the U compoenent
	 *         of vector flow, the V component, the correlation peak height, and
	 *         the color encoded direction of the flow.
					...
					for ( int i = ai.getAndIncrement(); i < npairs; i = ai.getAndIncrement() )
					{
						...
						for ( int x = 0; x <= image_width - winsize_x; x++ )
						{
							...
							for ( int y = 0; y <= image_height - winsize_y; y++ )
							{
								...
								if ( ( roi != null ) && ( !roi.contains( x + winsize_x / 2, y + winsize_y / 2 ) ) )
									continue;
								...
								pcm.swapQuadrants(); // centered in middle of
														// window
						...
						// snr_st.setPixels(new FloatProcessor(snr).getPixels(),
						// i + 1);
	 ...
	 * a 3x3 neighborhood around the maxima. The peak value (non-interpolated)
	 * is returned for futher filtering purpose. So far, there is no estimation
	 * of the signal/noise ratio calculated.
	 * 
	 * @param pcm
	 ...
	 * @param maxDistance
	 *            The max expected length of a vector
	 ...
	 * 
	 * @param ip
	 *            The ImageProcessor to draw the color circle in
	 */
...
//		cwcanvas.addMouseMotionListener(getColorMouseListener());
	...
	/**
	 * Creates the mouse listener that will "deconvolve" the color coded flow
	 * vector in an orientation and magnitude. It is not very clever, since the
	 * color was calculated from the flow vector in the method
	...
	final private void mask( final float[][] arr, final float[][] mask_arr, final float max_mask_value )
	{
		...
		for ( int i = 0; i < mask_arr.length; i++ )
		{
			...
			{
				if ( mask_arr[ i ][ j ] < val )
					arr[ i ][ j ] = 0.0f; // Could be Float.NaN but would have
											// messed with grayscales
			}
		}
	}
	 ...
	 * 
	 * @param first
	 *            The first index of the image pairs
	 * @param last
	 *            The max index of the image pairs
	 * @param step
	 *            Step from one pair to another
	 * @param jump
	 *            Step between back and front image
	 ...
	 * 
	 * @param image_pairs
	 *            The nx2 array of int specifying the image pairs.
	 */
	 ...
	 * 
	 * @param ws
	 *            The window size, specified by an enum
	 */
